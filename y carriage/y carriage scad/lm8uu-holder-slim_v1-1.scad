...
module mount_plate()
	...
	difference()
		{
		translate([0,0,3])
			cube([body_width+2*screw_head_dia+4*nut_surround_thickness,screw_head_dia+2*nut_surround_thickness,6], center=true);

		for(i=[-1,1])
			{
			translate([i*(body_width/2+nut_surround_thickness+screw_head_dia/2),0,-0.5])
				cylinder(r=screw_thread_dia/2, h=8, $fn=20);
			// nut trap
			translate([i*(body_width/2+nut_surround_thickness+screw_head_dia/2),0,3])
				rotate([0,0,30])
					cylinder(r=nut_dia/2, h=3,$fn=6);
			}
		}
	}
